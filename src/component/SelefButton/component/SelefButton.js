...
import { deleteStart, saveOrUpdateStart } from '../action';
...
const buttonType = {
    ...
    goto_delete: {
        handleClick(event) {
            ...
            confirm({
                ...
                cancelText: '取消',
                onOk() {
                    that.props.dispatch(deleteStart(linkUrl));
                }
            })
            // console.log(that);
    ...
    save_form: {
        render(props) {
            const { that } = props;
            const { disabled } = that.props;
            return <Button htmlType="submit" className={disabled ? "disabled" : ''} disabled={disabled ? true : false} type="primary" >{props.children}</Button>
        }
    },
    save_plist: {
        render(props) {
